...
5. Regenerate generated sources from the configured LibPhoneNumber vesion
   ```
   mvn generate-sources
   ```
    This updates 'src/main/java/de/telekom/phonenumbernormalizer/numberplans/GeneratedRegionDailingMetadata.java.


6. Run all unit test and check log messages if Google's LibPhoneNumber still is not correctly:
...
7. Remove ```-SNAPSHOT``` from version tag in [pom.xml](pom.xml)

8. Commit & Push the Snapshot with a message like:
   ```
   Use Google's LibPhoneNumber X.YY.ZZ and prepare release
   ```
   
9. Go to Github and create a pull request for the branch.

10. Wait until pull request passed all checks - merge or ask a maintainer to merge the pull request into main.

11. After merge has finished, draft a new Release. Use as tag the ```v```+ the version number of the pom, where you removed ```-SNAPSHOT```. As Release title use  ```Google's LibPhoneNumber X.YY.ZZ``` and add a message like:
    ```
...
12. Wait until the publishing action is finished (the project is automatically released to [Maven Central](https://central.sonatype.com/artifact/de.telekom.phonenumber/normalizer/))

13. Add ```-SNAPSHOT``` to version tag and increase its last number in [pom.xml](pom.xml)

14. Commit & Push the Snapshot with a message like:
    ```
    Start Snapshot NEW_VERSION after release OLD_VERSION
    ```

15. Go to Github and create a pull request for the branch.

16. Wait until pull request passed all checks - merge or ask a maintainer to merge the pull request into main.

17. Delete the branch ```phonelib/X_YY_ZZ```.
